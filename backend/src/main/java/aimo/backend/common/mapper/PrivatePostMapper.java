...
public class PrivatePostMapper {
	...
	public static PrivatePostResponse toResponse(PrivatePost privatePost) {
		return new PrivatePostResponse(privatePost.getId(), privatePost.getTitle(), privatePost.getSummaryAi(),
			privatePost.getStancePlaintiff(), privatePost.getStanceDefendant(), privatePost.getFaultRatePlaintiff(),
			privatePost.getFaultRateDefendant(), privatePost.getJudgement(), privatePost.getPublished());
	}

	public static SummaryAndJudgementRequest toSummaryAndJudgementRequest(JudgementToAiParameter parameter,
		Member member) {
		return new SummaryAndJudgementRequest(parameter.content(), member.getNickname(), member.getGender(),
			member.getBirthDate());
	}

	public static JudgementResponse toJudgementResponse(JudgementFromAiResponse judgementFromAi,
		Integer faultRatePlaintiff, Integer faultRateDefendant, JudgementToAiParameter parameter) {
		return new JudgementResponse(judgementFromAi.title(), judgementFromAi.summaryAi(),
			judgementFromAi.stancePlaintiff(), judgementFromAi.stanceDefendant(), judgementFromAi.judgement(),
			100 - faultRatePlaintiff, 100 - faultRateDefendant, parameter.originType());
	}

	public static JudgementParameter toJudgementParameter(JudgementResponse response) {
		Long memberId = MemberLoader.getMemberId();
		return new JudgementParameter(memberId, response.title(), response.summary(), response.stancePlaintiff(),
			response.stanceDefendant(), response.judgement(), response.faultRatePlaintiff(),
			response.faultRateDefendant(), response.originType());
	}

	public static JudgementToAiParameter toJudgementToAiParameter(JudgementToAiRequest judgementToAiRequest) {
		return new JudgementToAiParameter(MemberLoader.getMemberId(), judgementToAiRequest.content(),
	...
	public static FindPrivatePostParameter toFindPrivatePostParameter(FindPrivatePostRequest request) {
		return new FindPrivatePostParameter(MemberLoader.getMemberId(), request.privatePostId());
	}

	public static FindPrivatePostPreviewParameter toFindPrivatepostPreviewParameter(Pageable pageable) {
		return new FindPrivatePostPreviewParameter(MemberLoader.getMemberId(), pageable);
	}

	public static DeletePrivatePostParameter toDeletePrivatePostParameter(DeletePrivatePostRequest request) {
		return new DeletePrivatePostParameter(MemberLoader.getMemberId(), request.privatePostId());
	}
